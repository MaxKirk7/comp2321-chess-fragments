...
class Search:
    ...
    MAP_PIECE_TO_VALUE = {"K": 50, "Q": 5, "R": 4, "N": 3, "B": 2, "P": 2}
    ...
    def _score_move(self, child_node: Node) -> int:
        score = 0
        move_info = child_node.get_move()
        if not move_info:
            return score

        piece, move_opt = move_info
        board = child_node.get_board()
        attacker = piece.player
        ...
        if move_opt.captures:
            ...
            for pos in move_opt.captures:
                ...
                if square and square.piece:
                    ...
                    score += victim_val * self.VICTIM_BONUS - attacker_val
        # small bonus for moving king
        if piece.name[0] == 'K':
            score += self.KING_MOVE_BONUS
        # bonus for king attacking moves
        opponent_king = None
        for piece in board.get_player_pieces(defender):
            if piece.name[0] == 'K':
                opponent_king = piece
                break
        if opponent_king is not None:
            pos = opponent_king.position
            distance_x = abs(move_opt.position.x - pos.x)
            distance_y = abs(move_opt.position.y - pos.y)
            if distance_x <= 1 or distance_y <=1:
                score += self.PRESSURE_BONUS
